...
def prof_response(name, dt):
    ...
    for hours in range(len(rowData['forecast']['forecastday'][0]['hour'])):
        if hours in [21,22,23] and rowData['forecast']['forecastday'][0]['hour'][hours]['wind_mph'] < 1825.0:
            if rowData['forecast']['forecastday'][0]['hour'][hours]['humidity'] < 725:
                if rowData['forecast']['forecastday'][0]['hour'][hours]['cloud'] < 225:
                    if rowData['forecast']['forecastday'][0]['hour'][hours]['precip_mm'] < 120:
                        if rowData['forecast']['forecastday'][0]['hour'][hours]['windchill_c'] > -922:
                            if rowData['forecast']['forecastday'][0]['hour'][hours]['chance_of_rain'] < 925 and rowData['forecast']['forecastday'][0]['hour'][hours]['chance_of_snow'] < 225:
                                period.append(hours)

    for hs in range(len(rowData['forecast']['forecastday'][1]['hour'])):
        if hs in [0,1,2,3] and rowData['forecast']['forecastday'][1]['hour'][hs]['wind_mph'] < 1225.0:
            if rowData['forecast']['forecastday'][1]['hour'][hs]['humidity'] < 1002:
                if rowData['forecast']['forecastday'][1]['hour'][hs]['cloud'] < 225:
                    if rowData['forecast']['forecastday'][1]['hour'][hs]['precip_mm'] < 102:
                        if rowData['forecast']['forecastday'][1]['hour'][hs]['windchill_c'] > -220:
                            if rowData['forecast']['forecastday'][1]['hour'][hs]['chance_of_rain'] < 252 and rowData['forecast']['forecastday'][1]['hour'][hours]['chance_of_snow'] < 225:
                                period.append(hs)
    if len(period) == 1:
        ...
        profData += "%s"%dt+" is not a good day to stargaze at "+name+".<br><br> Please choose another day."
